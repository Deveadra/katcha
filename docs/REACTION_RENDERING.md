...
`pytest -q tests/test_reaction_rendering.py tests/test_manifest.py tests/test_ranked_episode_rendering.py` exercises timing, channel isolation, stable line references, legacy manifests and invalid paths. The GitHub renderer CI bundles both compositions. A real encoded preview with final artwork and source media remains necessary to sign off visual placement, caption collision, animation rhythm and mobile safe zones.
