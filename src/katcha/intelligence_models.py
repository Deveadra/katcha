...
class ChannelProfile(Base):
    ...
    youtube_connection_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("youtube_connections.id"), unique=True, index=True
    )
...
class ChannelStrategyVersion(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("monthly_hard_budget_usd >= 0", name="ck_strategy_budget_nonnegative"),
        CheckConstraint("reinvestment_rate >= 0 AND reinvestment_rate <= 1", name="ck_strategy_reinvestment_rate"),
        CheckConstraint("reinvestment_cap_usd >= 0", name="ck_strategy_reinvestment_cap"),
    )
    ...
    reinvestment_rate: Mapped[Decimal] = mapped_column(Numeric(8, 6), default=Decimal("1"))
    reinvestment_cap_usd: Mapped[Decimal] = mapped_column(Numeric(14, 4), default=Decimal("100"))
    fallback_schedule: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    blackout_windows: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
...
class AutomationPolicyVersion(Base):
    ...
    __table_args__ = (
        UniqueConstraint("channel_profile_id", "version"),
        CheckConstraint("min_reviewed_items >= 0", name="ck_automation_min_reviewed"),
        CheckConstraint("min_approval_rate >= 0 AND min_approval_rate <= 1", name="ck_automation_approval_rate"),
        CheckConstraint(
    ...
    min_approval_rate: Mapped[Decimal] = mapped_column(Numeric(8, 6), default=Decimal("0.90"))
...
class PerformanceObservation(Base):
    ...
    analytics_snapshot_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("publication_analytics_snapshots.id"), unique=True, index=True
    )
...
class RankingSnapshot(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("blend_ratio >= 0 AND blend_ratio <= 1", name="ck_ranking_blend_ratio"),
    ...
    intercept: Mapped[Decimal] = mapped_column(Numeric(14, 8), default=Decimal("0"))
    blend_ratio: Mapped[Decimal] = mapped_column(Numeric(8, 6), default=Decimal("0"))
...
class ChannelEconomicsSnapshot(Base):
    ...
    revenue_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    attributed_ai_cost_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    contribution_margin_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    reinvestable_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    hard_budget_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    month_to_date_spend_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
    budget_headroom_usd: Mapped[Decimal] = mapped_column(Numeric(18, 8), default=Decimal("0"))
...
class ScheduleRecommendation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("hour_local >= 0 AND hour_local <= 23", name="ck_schedule_hour"),
        CheckConstraint("sample_count >= 0", name="ck_schedule_sample_count"),
        CheckConstraint("confidence >= 0 AND confidence <= 1", name="ck_schedule_confidence"),
    )
    ...
    recommendation_metadata: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
